...
static sgx_status_t SGX_CDECL Enclave_ocall_file_exists(void* pms)
{
	ms_ocall_file_exists_t* ms = SGX_CAST(ms_ocall_file_exists_t*, pms);
	ocall_file_exists(ms->ms_filename, ms->ms_exists);
...
static sgx_status_t SGX_CDECL Enclave_ocall_read_file(void* pms)
{
	...
	ocall_read_file(ms->ms_filename, ms->ms_buffer, ms->ms_buffer_size, ms->ms_ret);
...
static sgx_status_t SGX_CDECL Enclave_ocall_write_file(void* pms)
{
	...
	ocall_write_file(ms->ms_filename, ms->ms_data, ms->ms_data_size, ms->ms_write_mode, ms->ms_ret);
...
static const struct {
	...
	14,
	{
		...
		(void*)Enclave_ocall_file_exists,
...
sgx_status_t ecall_add(sgx_enclave_id_t eid, int a, int b, int* result)
{
	sgx_status_t status;
	ms_ecall_add_t ms;
	ms.ms_a = a;
...
sgx_status_t ecall_create_wallet(sgx_enclave_id_t eid, unsigned char* ret_pub_key, unsigned char* ret_priv_key, size_t key_len, int wallet_id, int* ret)
{
	sgx_status_t status;
	ms_ecall_create_wallet_t ms;
	ms.ms_ret_pub_key = ret_pub_key;
	ms.ms_ret_priv_key = ret_priv_key;
	ms.ms_key_len = key_len;
	ms.ms_wallet_id = wallet_id;
	ms.ms_ret = ret;
	status = sgx_ecall(eid, 1, &ocall_table_Enclave, &ms);
	return status;
}

sgx_status_t ecall_deal_transaction(sgx_enclave_id_t eid, int from_id, int to_id, double amount, char* shuffled_output, size_t shuffled_output_size, char* encrypted_output, size_t encrypted_output_size, int* warning_sign, int* ret)
{
	sgx_status_t status;
	ms_ecall_deal_transaction_t ms;
	ms.ms_from_id = from_id;
	ms.ms_to_id = to_id;
	ms.ms_amount = amount;
	ms.ms_shuffled_output = shuffled_output;
	ms.ms_shuffled_output_size = shuffled_output_size;
	ms.ms_encrypted_output = encrypted_output;
	ms.ms_encrypted_output_size = encrypted_output_size;
	ms.ms_warning_sign = warning_sign;
	ms.ms_ret = ret;
	status = sgx_ecall(eid, 2, &ocall_table_Enclave, &ms);
	return status;
}

sgx_status_t ecall_decrypt_transaction_data(sgx_enclave_id_t eid, const char* encrypted_from, const char* encrypted_id, const char* encrypted_amount, size_t encrypted_size, int* from_id, int* to_id, double* amount, int* ret)
{
	sgx_status_t status;
	ms_ecall_decrypt_transaction_data_t ms;
	ms.ms_encrypted_from = encrypted_from;
	ms.ms_encrypted_id = encrypted_id;
	ms.ms_encrypted_amount = encrypted_amount;
...
sgx_status_t ecall_encrypt_transaction(sgx_enclave_id_t eid, int from_id, int to_id, double amount, int* ret)
{
	sgx_status_t status;
	ms_ecall_encrypt_transaction_t ms;
	ms.ms_from_id = from_id;
	ms.ms_to_id = to_id;
	ms.ms_amount = amount;
	ms.ms_ret = ret;
...
sgx_status_t ecall_transaction_warning(sgx_enclave_id_t eid, int from_id, int to_id, double amount, int* warning_sign, int* ret)
{
	sgx_status_t status;
	ms_ecall_transaction_warning_t ms;
	ms.ms_from_id = from_id;
	ms.ms_to_id = to_id;
	ms.ms_amount = amount;
	ms.ms_warning_sign = warning_sign;
